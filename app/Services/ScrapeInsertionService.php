...
//            dd($searchData);
    ...
    public static function insertProductNew($slug)
    {
...
//
//        if ($product->scraped_date && Carbon::parse($product->scraped_date)->addHours(3) > now()) {
//            return [];
//        }
...
//            dd($searchData);
            ...
            $colors         = [];
            $choice_options = [];
            //Default olaraq selected = true olan goturub colorunu onunda sizini gotur
            $photos = [];
            ...
            foreach ($searchData['attributes'] as $attribute) {
                $color = $attribute['color_name'] ?? null;
                $size  = $attribute['size_name'] ?? null;
//                foreach ($searchData['colors'] as $color) {
                $_color  = null;
                $__color = null;
                if (isset($attribute['color_name'])) {

                    $_color  = Color::where('code', $color)->first();
                    $__color = str_replace('/', '-', $color);
                ...
                $_category_ids = AttributeCategory::whereIn('category_id', $category_ids)
                    ->pluck('category_id')
                    ->toArray();

                $missing_category_ids = array_diff($category_ids, $_category_ids);
                $str                  = '';
                if (isset($attribute['size_name'])) {
                    $choice_options = [
                        'attribute_id' => 1,
                        'values'       => []
                    ];

                    $__size = isset($attribute['size_name']) ? $attribute['size_name'] : '';
                    $__size = explode(' ', $__size);

                    if (count($__size) > 0) {
                        foreach ($__size as $key => $___size) {
                            if ($key == 0) {
                                $str .= $str . '-' . $___size;
                            } else {
                                $str .= $___size;
                ...
                $choice_options['values'][] = $attribute['size_name'];
                $productStock               = new ProductStock();
                $attributeValue             = AttributeValue::where('value', $attribute['size_name'])->first();

                if (!$attributeValue) {
                    $attributeValue               = new AttributeValue();
                    $attributeValue->attribute_id = 1;
                    $attributeValue->value        = $attribute['size_name'];
                    $attributeValue->save();

                }

                $productStock->product_id      = $product->id;
                $productStock->scraped_item_id = $attribute['asin'];
                $productStock->variant         = $variant;
                $productStock->sku             = $attribute['asin'];
                $productStock->price           = $price;
                $productStock->qty             = 100;
                $productStock->datas           = $attribute;
                $productStock->save();
...
//            dd($e->getLine(), $e->getMessage());
    ...
    public static function insertProductOld($slug)
    {
...
//                foreach ($searchData['colors'] as $color) {
                ...
                $_color   = Color::where('code', $color['color'])->first();
                $__color  = str_replace('/', '-', $color['color']);
                $__color  = str_replace(' ', '-', $__color);
                $colors[] = $__color;
                ...
                if (count($searchData['sizes']) > 0) {
                    ...
                    foreach ($searchData['sizes'] as $size) {
                        $scraped_item_id = $color['asin'] . "-" . $size['asin'];
//                        foreach ($missing_category_ids as $_category_id) {
//                            $AttributeCategory =  new AttributeCategory();
//                            $AttributeCategory->category_id = $_category_id;
//                            $AttributeCategory->attribute_id = 1;
//                            $AttributeCategory->save();
//                        }

                        $__size = isset($size['size']) ? $size['size'] : '';
                        $__size = explode(' ', $__size);
                        $str    = '';
                        if (count($__size) > 0) {
                            foreach ($__size as $key => $___size) {
...
//                        dd($product->id);
                        ...
                        $productStock               = ProductStock::where('product_id', $product->id)->where('variant',
                            $variant)->first();
                        $choice_options['values'][] = $size['size'];
                        if (!$productStock) {
                            $productStock = new ProductStock();
                        }
                        $attributeValue = AttributeValue::where('value', $size['size'])->first();

                        if (!$attributeValue) {
                            $attributeValue               = new AttributeValue();
                            $attributeValue->attribute_id = 1;
                            $attributeValue->value        = $size['size'];
                            $attributeValue->save();
                        ...
                        $productStock->variant         = $__color . $str;
