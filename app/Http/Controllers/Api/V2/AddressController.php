...
//        return new CitiesCollection(City::where('status', 1)->get());
        return new CitiesCollection(City::when(request("name"),function($query,$name){
            return $query->where("name","like","%$name%");
        })->when(request("state_id"),function($query,$state_id){
...
//        return new StatesCollection(State::where('status', 1)->get());
        return new StatesCollection(State::when(request("name"),function($query,$name){
            return $query->where("name","like","%$name%");
...
//        $country_query = Country::where('status', 1);
        ...
        if ($request->name != "" || $request->name != null) {
            $country_query->where('name', 'like', '%' . $request->name . '%')->orWhere("code","like","%$request->name%");
        }
