...
class ProductController extends Controller
{
    ...
    public function getPrice(Request $request)
    {
        ...
        if ($product->wholesale_product) {
            $wholesalePrice = $product_stock->wholesalePrices->where('min_qty', '<=', $quantity)->where('max_qty', '>=', $quantity)->first() ?? 0;
            if ($wholesalePrice) {
                $price = $wholesalePrice->price;
            }
        }

        $stock_qty = $product_stock->qty ?? 0;
        $stock_txt = $product_stock->qty ?? 0;
        ...
        return response()->json(
            ...
            [
                ...
                'data' => [
                    ...
                    'max_limit' => $max_limit,
                    'in_stock' => $in_stock,
                    'image' => !isset($product_stock->image) || $product_stock->image == null ? "" : uploaded_asset($product_stock->image)
                ]
