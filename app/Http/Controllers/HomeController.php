...
class HomeController extends Controller
{
    ...
    public function load_newest_product_section()
    {
        $newest_products = Cache::remember('newest_products', 3600, function () {
            return filter_products(Product::latest())->limit(24)->get();
        });
    ...
    {
        return view("frontend.best_selling");
    }
...
//        dd('auth.' . get_setting('authentication_layout_select') . '.user_registration');
    ...
    public function product(Request $request, $slug)
    {
        ...
        $searchData  = ScrapeInsertionService::insertProduct($slug);

        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','main_category','main_category.sizeChart')
            ->where('auction_product', 0)
            ->where('slug', $slug)
            ->where('approved', 1)
            ->first();
//dd($detailedProduct->toArray());
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            $decodedColors = is_string($detailedProduct->colors) ? json_decode($detailedProduct->colors, true) : [];
    ...
    public function variant_price(Request $request)
    {
        ...
        $price = $product_stock->price ?? null;
