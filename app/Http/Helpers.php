...
if (!function_exists('is_dev')) {
   function is_dev($ip = '94.20.38.158'){
       $allowed_ips = [
            '94.20.38.158',
            '5.134.56.55'
        ];
...
if (!function_exists('client_ip')) {
    function client_ip(){
        return $_SERVER['HTTP_CF_CONNECTING_IP'] ?? null;
    }
}


if (!function_exists('liveDebug')) {
    function liveDebug($data,$params = [],$client_ip = '94.20.38.158')
    {
        $allowed_ips = [
            '94.20.38.158',
            '5.134.56.55'
        ...
        if ((in_array(client_ip(),$allowed_ips)) || env('APP_ENV') === 'local') {
            if($params) {
                dd([
                    "data" => $data,
                    "params" => $params
...
if (!function_exists('sendTelegram')) {
    function sendTelegram($errorMessage)
    {
        $errorMessage = "<pre>$errorMessage</pre>";
        $botToken     = env('TELEGRAM_CHAT_TOKEN');
        $website      = "https://api.telegram.org/bot" . $botToken;
        $chatId       = env('TELEGRAM_CHAT_ID');  // Receiver Chat ID
        file_get_contents($website . "/sendMessage?chat_id=" . $chatId . "&parse_mode=html&text=" . $errorMessage);
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($product, $formatted = true)
    {
        ...
        if (isset($product->discount_start_date) && $product->discount_start_date == null) {
            $discount_applicable = true;
        } elseif (
            isset($product->discount_start_date) &&
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        ...
        if ($discount_applicable) {
            if (isset($product->discount_type) && $product->discount_type == 'percent') {
                $price -= ($price * $product->discount) / 100;
            } elseif (isset($product->discount_type) && $product->discount_type == 'amount') {
                $price -= $product->discount;
            }
        }
...
if (!function_exists('get_single_color_image')) {
    function get_single_color_image($color)
    {
        $color_query = Color::query();
        return $color_query->where('code', $color)->first()->image;
    }
}
