...
<div class="col-lg-8 mx-auto">
	<div class="card">
		<div class="card-body p-0">
			<form class="p-4" action="{{ route('admin.auto-models.update', $model->id) }}" method="POST">
					...
					<div class="col-md-9">
						<select class="form-control aiz-selectpicker" name="brand_id" id="brand_id" data-live-search="true" required>
							...
							@foreach ($brands as $brand)
								<option value="{{ $brand->id }}" @if($model->brand_id == $brand->id) selected @endif>
				...
				<div class="form-group row">
					...
					<div class="col-md-9">
						<input type="text" name="name" value="{{ $model->name }}" class="form-control" placeholder="{{ translate('Model Name') }}" required>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-md-9">
						<select class="form-control aiz-selectpicker" name="auto_model_group_id" data-live-search="true">
							...
							@foreach ($modelGroups as $group)
								<option value="{{ $group->id }}" @if($model->auto_model_group_id == $group->id) selected @endif>
									{{ $group->name }}
								</option>
							@endforeach
						</select>
				...
				<div class="form-group row">
					...
					<div class="col-md-9">
						<input type="number" name="year_from" value="{{ $model->year_from }}" class="form-control" placeholder="{{ translate('Year From') }}" min="1900" max="2100">
					</div>
				</div>
				<div class="form-group row">
					<label class="col-md-3 col-from-label">{{ translate('Year To') }}</label>
					<div class="col-md-9">
						<input type="number" name="year_to" value="{{ $model->year_to }}" class="form-control" placeholder="{{ translate('Year To') }}" min="1900" max="2100">
					</div>
				</div>
